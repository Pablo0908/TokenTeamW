...
onMounted(async () => {
  ...
  const init = () => {
    if (!window.google?.accounts?.id || !gsiRef.value) return
    window.google.accounts.id.initialize({
      client_id: clientId,
      callback: ({ credential }) => emit('credential', credential),
    })
    window.google.accounts.id.renderButton(gsiRef.value, {
      theme: 'outline',
      size: 'large',
      width: gsiRef.value.parentElement?.offsetWidth ?? 360,
      text: 'continue_with',
      shape: 'rectangular',
    })
  }
...
<template>
  ...
  <div class="relative w-full" style="height:40px">
    ...
    <!-- Google's official button: opacity-0 so invisible, but still receives clicks -->
    <div ref="gsiRef" class="absolute inset-0 overflow-hidden opacity-0" />
  </div>
</template>
